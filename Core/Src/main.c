...
void StartDefaultTask(void *argument)
{
  ...
  for (;;)
  {
    ...
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 0);   // 0%
      osDelay(10000);
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 100);   // 25%
      osDelay(10000);

    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 840);   // 25%
    osDelay(10000);

*/
    //__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 1680);  // 50%
    // osDelay(10000);
    /*
         __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 2520);  // 75%
         osDelay(10000);

         __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 3359);  // 100%
         osDelay(10000);
    */
    // osDelay(1000);
    ...
    //   fanRpm = pulses * 30; // 2 pulses per revolution, 1 second window

    // char msg[64];
    // snprintf(msg, sizeof(msg), "Pulses=%lu RPM=%lu\r\n", pulses, fanRpm);
    // HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    // printf("ESTOP=%d\r\n",
    //          HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_0));
    //     char msg[64];

    //       snprintf(msg, sizeof(msg), "ESTOP=%d\r\n",
    //   HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_0));
...
void StartTelemetryTask(void *argument)
{
...
#else
  HAL_UART_Transmit(&huart2,
                    (uint8_t *)"Vibration sensor disabled\r\n",
                    27,
                    HAL_MAX_DELAY);
#endif
  Fan_Stop();
  for (;;)
  {
    ...
    if (status == osOK)
    {
      latestTelemetry.vibrationX_mg = 0;
      latestTelemetry.vibrationY_mg = 0;
      latestTelemetry.vibrationZ_mg = 0;
      latestTelemetry.vibration_level_mg = 0;
      osMutexRelease(telemetryMutex);
    }
#endif
